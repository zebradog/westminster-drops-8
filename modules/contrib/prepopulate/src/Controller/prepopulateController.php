    ...
    if (is_array($request_slice)) {
      foreach (array_keys($request_slice) as $request_variable) {
        if (isset($form[$request_variable])) {
          $type = (isset($form[$request_variable]['widget'][0]['value']['#type'])) ? $form[$request_variable]['widget'][0]['value']['#type'] : $form[$request_variable]['widget']['#type'];
      ...
      $widget = $form['widget'];
      if (empty($widget) || (isset($widget[0]) && $widget[0]['value']['#type'] == 'markup')) {
